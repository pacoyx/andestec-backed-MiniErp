...
namespace WA_AndesTec.Controllers
{
    ...
    public class MA_PROVIDERController : ApiController
    {
        ...
        [HttpPost, Route("")]
        public string Post([FromBody]EMA_PROVIDER value)
        {
            return negocio.Registrar(value);
        }

        [HttpPut, Route("")]
        public string Put([FromBody]EMA_PROVIDER value)
        {
            return negocio.Registrar(value);
        }
        ...
        {
            return negocio.Eliminar(new EMA_PROVIDER { ID_COMPANY = ide, ID_PROVIDER = id });
        }
    }
}
